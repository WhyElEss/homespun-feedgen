...
- **Diagnostics** — `whyNot.ts` explains stage-by-stage why a given post did or didn't get into your feed; `probeCorpus.ts` measures a candidate filter against the posts a feed already holds, before you save it; `probeJetstream.js` measures the lag of all public Jetstream instances (yes, one of them can silently fall an hour behind); `probeWatchdog.ts` proves the ingest still recovers from a connection that dies without saying so.
...
# measure a candidate pattern against what a feed already holds, before saving it
docker compose run --rm feedgen yarn probeCorpus --feed <rkey> --pattern '\\bscreen\\s?shots?\\b'
docker compose run --rm feedgen yarn probeCorpus --feed <rkey> --spec /data/probe.json
...
# feed stopped filling with the service still running? prove the reconnect works
...
`probeCorpus` answers the question every filter edit starts with: *how many stored posts would this take, and which ones?* Give it one `--pattern` or a `--spec` file listing several, and it reports each candidate's hit count, the substring it matched and the caption it matched in, plus whether a sweep that size would be withheld by the caps above. Three things in it are there because they were learned the hard way. It searches the same haystack the service does, **alt text included** — the offender is often a post with no text at all. `--against <substring>` names a live exclude block by its comment and reports anything that rule catches which your candidates do not, because a widened pattern that quietly drops an arm is a regression nothing else would tell you about. And a spec can carry `mustSurvive` / `mustDie` lists, which is how you find out that the tag you were sure meant "artwork" is one photographers use: the tool exits non-zero when an expectation is not met, so it is usable in a script. What it cannot do is the standing limit of measuring against stored posts — it can size a narrowing or an exclude exactly, and it can say nothing at all about what a *widened include* would newly let in, because those posts were never stored.
...
`probeWatchdog` answers a question the logs cannot: *if the connection died right now, would anything notice?* A dropped link does not always produce a socket event — when the peer sends neither FIN nor RST the connection sits half-open, `'close'` never fires, and a subscription that reconnects only on `'close'` will sit there healthy and idle forever. That is not hypothetical; it cost this project 10.5 hours of ingest, with the process up the whole time and its hourly housekeeping still logging on the dot. The probe brings its own WebSocket server and runs two phases against one subscription: first the server talks, and nothing may be cut, because a watchdog that churns a working connection is worse than no watchdog; then the server goes silent **without closing anything**, and the reconnect must happen anyway. It takes a little over two minutes and exits non-zero if either half fails. Run it after touching anything in `src/subscription.ts` — the reconnect path is the one piece of this service that no amount of normal traffic will ever exercise.
